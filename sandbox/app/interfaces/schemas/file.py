...
class FileReadRequest(BaseModel):
    ...
    end_line: Optional[int] = Field(
        default=None, description="(可选)结束行号, 不包含该行"
    )
    sudo: Optional[bool] = Field(default=False, description="(可选)是否使用sudo权限")
    max_length: Optional[int] = Field(
        default=10000, description="(可选)要返回的内容的最大长度"
    )
...
class FileWriteRequest(BaseModel):
    ...
    leading_newline: Optional[bool] = Field(
        default=False, description="(可选)是否在内容开头添加前置空行"
    )
    trailing_newline: Optional[bool] = Field(
        default=False, description="(可选)是否在内容结尾添加前置空行"
    )
    sudo: Optional[bool] = Field(default=False, description="(可选)是否使用sudo权限")

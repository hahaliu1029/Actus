  ...
  const steps = [
    {
      id: 1,
      status: 'completed',
      description: '使用搜索工具查找第四季度（假设为当前或最近一年的10月至12月）国内人工智能的最新新闻和发展动态，使用中文关键词如\'第四季度 AI 发展 新闻 中国\'。',
    },
    {
      id: 2,
      status: 'running',
      description: '使用搜索工具查找第四季度国际人工智能的最新新闻和发展动态，使用英文关键词如\'Q4 AI developments 2023 international\'或类似，以覆盖全球范围。',
    },
    {
      id: 3,
      status: 'running',
      description: '从搜索结果中选择多个相关URL，并使用浏览器工具访问这些页面，提取页面内容以获取详细信息，包括新闻标题、日期和来源。',
    },
  ]
  ...
  return (
    <div className={cn('bg-card rounded-xl border border-border', className)}>
      ...
      >
        ...
        {/* 右侧操作按钮&步骤信息 */}
        <div
          className="flex h-full justify-center gap-2 flex-shrink-0 items-center py-2.5"
        >
          <span className="text-xs text-muted-foreground">1 / 5</span>
          <ChevronUp className="text-foreground/85" size={16}/>
        </div>
      </div>}
      {/* 展开状态 */}
      {isExpanded && <div
        className="flex flex-col py-4 rounded-xl z-99"
      >
        {/* 顶部留白+按钮 */}
          ...
          <div className="bg-muted rounded-lg px-2 py-3">
            ...
            <div className="max-h-80 overflow-y-auto">
              {steps.map(step => (
                <div
                  key={step.id}
                  className="flex items-center text-muted-foreground text-sm gap-2.5 w-full px-4 py-2 truncate"
                >
                  {/* 图标 */}
                  {

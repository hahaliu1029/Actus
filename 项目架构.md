...
- 目录型 Skill 的权威文件位于 API 容器：`/app/data/skills/<skill_id>/bundle`。
- 每个会话启动时执行一次同步生命周期：
  1. 先基于空消息 selector 选出初始子集（默认 `top_k=12`）并前台阻塞同步。
  2. 再后台并发补齐本会话冻结技能池中的剩余 Skill（并发固定 4）。
- 会话期间采用静态版本语义：不做版本时效检查；Skill 更新在下一会话生效。
- native tool 在 `entry.exec_dir` 缺失时，默认在 sandbox 目录 `/home/ubuntu/workspace/.skills/<skill_id>` 执行。
- native 调用前若同步失败或执行目录不存在，会直接返回错误，不进行盲执行。

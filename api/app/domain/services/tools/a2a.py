...
A2A_DISCOVERY_HTTP_TIMEOUT_SECONDS = 8
A2A_AGENT_CARD_TIMEOUT_SECONDS = 8
A2A_TOOL_INITIALIZE_TIMEOUT_SECONDS = 10
...
async def _run_with_timeout_isolated(coro, timeout_seconds: int) -> None:
    """在当前Task中执行协程并限制超时，避免跨Task清理导致资源无法释放。"""
    async with asyncio.timeout(timeout_seconds):
        await coro


def _filter_enabled_a2a_config(a2a_config: Optional[A2AConfig]) -> A2AConfig:
    """仅保留启用的A2A服务，避免禁用/无效服务阻塞任务启动。"""
    if not a2a_config:
        return A2AConfig(a2a_servers=[])
    return A2AConfig(
        a2a_servers=[server for server in a2a_config.a2a_servers if server.enabled]
    )
...
class A2AClientManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            servers = self._a2a_config.a2a_servers if self._a2a_config else []
            if not servers:
                logger.info("当前无启用的A2A服务，跳过初始化")
                self._initialized = True
                return

            # 3.初始化httpx客户端
            self._httpx_client = await self._exit_stack.enter_async_context(
                httpx.AsyncClient(timeout=A2A_DISCOVERY_HTTP_TIMEOUT_SECONDS),
            )

            # 4.记录日志并连接所有配置的a2a服务获取卡片信息
            logger.info(f"加载{len(servers)}个A2A服务")
    ...
    async def _get_a2a_agent_cards(self) -> None:
        ...
        servers = self._a2a_config.a2a_servers if self._a2a_config else []
        # 1.循环遍历所有的a2a服务
        for a2a_server_config in servers:
            if not a2a_server_config.enabled:
                logger.info(f"跳过未启用的A2A服务[{a2a_server_config.id}]")
                continue
            try:
                # 2.调用httpx客户端发起请求
                agent_card_response = await asyncio.wait_for(
                    self._httpx_client.get(
                        f"{a2a_server_config.base_url}/.well-known/agent-card.json",
                        timeout=A2A_AGENT_CARD_TIMEOUT_SECONDS,
                    ),
                    timeout=A2A_AGENT_CARD_TIMEOUT_SECONDS,
                )
    ...
    async def invoke(self, agent_id: str, query: str) -> ToolResult:
        ...
        if not self._httpx_client:
            return ToolResult(success=False, message="A2A客户端未初始化")
        ...
        try:
            ...
            return ToolResult(
                success=False, message=f"调用远程Agent[{agent_id}:{url}]出错: {str(e)}"
            )
...
class A2ATool(BaseTool):
    ...
    async def initialize(self, a2a_config: Optional[A2AConfig] = None) -> None:
        ...
        if not self._initialized:
            filtered_config = _filter_enabled_a2a_config(a2a_config)
            if not filtered_config.a2a_servers:
                logger.info("当前无启用的A2A服务，跳过初始化")
                self.manager = None
                self._initialized = True
                return
            ...
            # 2.初始化A2A客户端管理器
            self.manager = A2AClientManager(filtered_config)
            try:
                await _run_with_timeout_isolated(
                    self.manager.initialize(),
                    timeout_seconds=A2A_TOOL_INITIALIZE_TIMEOUT_SECONDS,
                )
            except BaseException as e:
                if _is_fatal_error(e):
                    raise
                logger.warning(f"A2A工具包初始化失败，降级继续执行主流程: {str(e)}")
                if self.manager:
                    try:
                        await asyncio.shield(self.manager.cleanup())
                    except BaseException as cleanup_error:
                        if _is_fatal_error(cleanup_error):
                            raise
                        logger.warning(
                            f"A2A工具包初始化失败后的清理异常（已忽略）: {cleanup_error}"
                        )
                self.manager = None
            finally:
                self._initialized = True
